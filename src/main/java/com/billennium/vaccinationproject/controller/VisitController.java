...
public class VisitController {
    ...
    public ResponseEntity<List<VisitDto>> getAllVisits (@RequestParam(name = "page", defaultValue = "0") int page,
                                                        @RequestParam(name = "size", defaultValue = "10") int size,
                                                        @RequestParam(name = "sortBy", defaultValue = "id") String sortBy) {
        return visitService.getAllVisits(page, size, sortBy);
    }
    ...
    public ResponseEntity<List<VisitDto>> getAllVaccinationDates (@RequestParam(required = false, name = "fromDate") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
                                                                  ...
                                                                  @RequestParam(name = "size", defaultValue = "10") int size,
                                                                  @RequestParam(name = "sortBy", defaultValue = "id") String sortBy) {
        return visitService.getAllVaccinationDates(fromDate, toDate, city, manufacturer, page, size, sortBy);
    }
    ...
    public ResponseEntity<?> deleteOldAndUnusedVisits (HttpServletRequest request) {
        String schedulerPassword = "blank";

        try {
            schedulerPassword = request.getHeader( "SchedulerPassword" );
        } catch (NullPointerException e){
            throw new ApiRequestException("Missing authentication header for scheduler!", HttpStatus.UNAUTHORIZED);
        }
        if(schedulerPassword.equals("af64csyertrsvyvtdyhfg")) {
